...
After any crash or editor switch:
1. Check git status is clean (commit/stash if not)
2. Verify `/framework/orch/task_ledger.json` checkpoint ≤ Phase 2 unless `build_manifest.sha256` exists
3. Clear stale session lock if >90s old
4. Run `./orchctl verify` to rebuild state
5. Use `./orchctl resume` to continue from safe checkpoint
...
---
*Framework Version: 3.1.1 FINAL | Status: Master-Ready | Reliability: 9.5/10 | Crash-Resilient*